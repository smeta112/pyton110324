...
#grid() - column(, row(, columnspan(объединение нескольких ячеек в одну, rowspan(, ipadx/ipady(отступ в кнопке), padx/pady(отступ за кнопкой), sticky(north, e , s, w, ne, nw...)#(выравнит элемент в ячейке)
#f' - форматирование
for r in range(3):
    for c in range(3):
